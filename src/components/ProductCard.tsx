...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <tr className="border-b border-[#eee] hover:bg-[#fafafa] group">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <Link
              ...
              className="font-medium text-[#1a5dab] text-sm"
            >
              {product.name}
            </Link>
            <p className="text-[11px] text-[#999] leading-tight">{product.unit}</p>
          </div>
        </div>
      </td>
        ...
        return (
          <td
            ...
            className={`py-2 text-right tabular-nums whitespace-nowrap text-sm px-1 ${
              isCheapest
                ? "font-bold text-[#2e7d32] bg-[#f1f8e9]"
                : "text-[#333]"
            }`}
          >
            {pp ? (
              ...
              <span className="text-[#ccc]">&ndash;</span>
            )}
          </td>
        );
      })}
