...
export default function HomePage() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4">
      {/* Ticker bar */}
      <div className="flex items-center gap-4 py-2.5 border-b border-[#e0e0e0] text-xs text-[#555] overflow-x-auto">
        <span className="shrink-0 font-semibold text-[#222]">
          Today&apos;s prices
        </span>
        ...
        {storeTotals.slice(0, 3).map(({ store }, i) => (
          <Link
            ...
            className="shrink-0 flex items-center gap-1 no-underline hover:no-underline text-[#555] hover:text-[#222]"
          >
            <StoreBadge store={store} />
            <span>{store.name}</span>
            {i === 0 && (
              <span className="text-[#2e7d32] font-semibold">(cheapest overall)</span>
            )}
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 py-5">
        ...
        <div>
          <div className="flex items-baseline justify-between mb-3">
            ...
            <span className="text-xs text-[#999]">{hotDeals.length} deals</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#e5e5e5] border border-[#e5e5e5]">
            {hotDeals.map((product) => {
              ...
              return (
                <Link
                  ...
                  className="bg-white p-3 flex flex-col items-center text-center no-underline hover:no-underline hover:bg-[#fffde7] transition-colors relative group"
                >
                  ...
                  <ProductImage
                    ...
                    size={52}
                  />
                  <p className="text-[11px] font-medium text-[#333] mt-2 leading-tight line-clamp-2 group-hover:text-[#b71c1c]">
                    {product.name}
                  </p>
                  <p className="text-[10px] text-[#999] mb-1">{product.unit}</p>
                  <p className="text-base font-extrabold text-[#b71c1c] tabular-nums">
                    KSh {lowest.price}
                  </p>
                  {saleEntry?.originalPrice && (
                    <p className="text-[10px] text-[#999] line-through tabular-nums">
        ...
        <div className="space-y-5">
          <div>
            <h2 className="text-sm font-bold text-[#222] mb-2">
              Biggest Price Gaps
            </h2>
            <p className="text-[11px] text-[#888] mb-3">
              Products where shopping around saves you the most.
            </p>
            <div className="border border-[#e0e0e0] divide-y divide-[#eee]">
              {topSavers.map((product) => {
                ...
                return (
                  <Link
                    key={product.id}
                    href={`/compare?product=${product.id}`}
                    className="flex items-center gap-2.5 px-3 py-2.5 bg-white hover:bg-[#f9f9f9] no-underline hover:no-underline transition-colors"
                  >
                    <ProductImage
                      src={product.image}
                      alt={product.name}
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2 no-underline hover:no-underline transition-colors ${
                      i === 0
                        ? "bg-[#f1f8e9] hover:bg-[#e8f5e9]"
                        : "bg-white hover:bg-[#f9f9f9]"
                    }`}
                  >
                    <span className="text-xs font-bold text-[#888] w-4 text-right tabular-nums">
                      {i + 1}
                    </span>
                    <StoreBadge store={store} />
                    <span className="text-sm text-[#222] font-medium flex-1">
                      {store.name}
                    </span>
      ...
      {/* Divider */}
      <div className="border-t border-[#ddd] mb-4" />
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-3">
        ...
        <select
          ...
          className="px-3 py-2 border border-[#ccc] rounded text-sm bg-white cursor-pointer focus:outline-none focus:border-[#999]"
        >
      ...
      {(search || category) && (
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-xs text-[#b71c1c] hover:underline cursor-pointer"
          >
            Clear
          </button>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-10 text-center">
          <p className="text-sm text-[#999] mb-2">No products match your filters.</p>
          <button
            onClick={() => { setSearch(""); setCategory(null); }}
            className="text-sm text-[#b71c1c] hover:underline cursor-pointer"
          >
            Reset filters
          </button>
        </div>
        ...
        <>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2 border-[#ddd] text-xs text-[#888] uppercase tracking-wide">
                  <th className="py-2 text-left font-medium">Product</th>
                  {supermarkets.map((store) => (
                    <th key={store.id} className="py-2 text-right font-medium px-1">
                      <Link
                        href={`/supermarket?id=${store.id}`}
                        className="inline-flex items-center gap-1 text-[#888] hover:text-[#333] no-underline hover:no-underline"
                      >
                        <StoreBadge store={store} />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <Link
                            href={`/compare?product=${product.id}`}
                            className="text-sm font-medium text-[#333] hover:text-[#b71c1c]"
                          >
                            {product.name}
                          </Link>
                        ...
                        {qty > 0 ? (
                          ...
                          <button
                            ...
                            className="text-xs text-[#333] border border-[#ccc] hover:border-[#999] bg-white hover:bg-[#f5f5f5] px-3 py-1.5 rounded cursor-pointer shrink-0 font-medium transition-colors"
                          >
                            + Add
                          </button>
                        )}
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {pp ? pp.price : "\u2013"}
                            {pp?.onSale && (
                              <span className="text-[#c62828] ml-0.5">*</span>
                            )}
                          </span>
                        </div>
                      );
