  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto px-4 py-12 text-center">
        ...
        <p className="text-sm text-[#666] mb-4">
          Your order <strong>{orderNo}</strong> has been placed at{" "}
          <strong>{store.name}</strong>.
        </p>
        <div className="bg-[#f7f7f7] border border-[#e0e0e0] rounded p-4 text-left text-sm text-[#555] mb-6 mx-auto max-w-sm">
          <div className="flex justify-between mb-1">
            <span>Order number</span>
            <span className="font-semibold text-[#222]">{orderNo}</span>
          </div>
          <div className="flex justify-between mb-1">
            <span>Store</span>
            <span className="font-medium text-[#222]">{store.name}</span>
          </div>
          <div className="flex justify-between mb-1">
            <span>Items</span>
          ...
          <div className="flex justify-between mb-1">
            <span>Payment</span>
            <span className="capitalize">{form.payment === "mpesa" ? "M-Pesa" : form.payment === "card" ? "Card" : "Cash on delivery"}</span>
          </div>
          <div className="flex justify-between border-t border-[#ddd] pt-1.5 mt-1.5 font-bold text-[#222]">
            <span>Total</span>
            <span>KSh {orderTotal.toLocaleString()}</span>
          </div>
        </div>
        <p className="text-xs text-[#999] mb-5">
          A confirmation will be sent to <strong>{form.email || form.phone}</strong>.
        </p>
        <Link
          href="/"
          className="text-sm text-[#1a5dab]"
        >
